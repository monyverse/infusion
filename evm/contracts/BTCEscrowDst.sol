 ...
 */
contract BTCEscrowDst is Ownable, ReentrancyGuard {
    ...
    // State variables (not immutable since they need to be set in initialize)
    bytes32 public orderHash;
    bytes32 public hashlock;
    uint256 public maker;
    uint256 public taker;
    ...
    /**
     * @dev Withdraw funds by revealing the secret (for BTC→EVM swaps)
    ...
    function withdraw(bytes32 secret) external nonReentrant {
        ...
        // Transfer funds to maker (in BTC→EVM, maker receives EVM tokens)
        ...
        emit Withdrawal(orderHash, makerAddress, totalAmount, secret);
    }

    /**
     * @dev Refund funds to taker after cancellation period
     */
    function refund() external nonReentrant {
        ...
        // Transfer funds back to taker (in BTC→EVM, taker gets refund)
        ...
        emit Refund(orderHash, takerAddress, totalAmount);
    }
