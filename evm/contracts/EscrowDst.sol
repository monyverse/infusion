...
contract EscrowDst is Escrow, IEscrowDst {
    ...
    modifier onlyValidSecret(bytes32 secret, Immutables calldata immutables) {
        _validateSecret(secret, immutables.hashlock);
        _;
    }
    ...
    /**
     * @dev Cancel the escrow and return tokens to taker
     */
    ...
    /**
     * @dev Rescue funds from the escrow
     */
    function _rescueFunds(address token, uint256 amount, Immutables calldata immutables) internal {
        if (token == address(0)) {
